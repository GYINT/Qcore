...
# 覆盖：8 引擎 + v82 + 路由黄金 + 守卫 10 检 + 双绑 + 词源语义检测 + 热词生命周期 + 实体层
...
echo "=== [3/10] 路由黄金用例（P0 歧义回归护栏）==="
$PY tests/qcm_router_golden_test.py

echo "=== [4/10] 守卫 10 检（配置一致性）==="
$PY scripts/config_sync.py --check

echo "=== [5/10] Manifest 双绑验证 ==="
$PY scripts/sync_manifest.py

echo "=== [6/10] 词源同类语义检测（V8.4 · 0 严重即绿）==="
$PY scripts/semantic_audit.py --check

echo "=== [7/10] 热词生命周期健康（容量/状态机）==="
$PY scripts/keyword_lifecycle.py --check

echo "=== [8/10] 实体层与知识库同步 ==="
$PY scripts/extract_entities.py --check

echo "=== [9/10] 全维度边界 + 压力测试（V8.4 · 输入/容量/阈值/实体/并发）==="
$PY tests/qcm_boundary_test.py
$PY tests/qcm_stress_test.py

echo "=== [10/10] 歧义消解三级链 + 回灌闭环（V8.4 · 无 Key 零回归）==="
